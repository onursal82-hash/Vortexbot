...
def calculate_dca_logic(user_email, bot, current_price):
    ...
    try:
        ...
        # 3. Take Profit Logic (Immediate Execution)
        tp_target = dca_config.get('take_profit', 1.5)
        
        should_sell = False
        
        # Strict comparison: if pnl_percent >= take_profit -> execute sell immediately
        if pnl_percent >= tp_target:
            should_sell = True
            # Remove trailing logic completely as requested
        ...
        # 4. Stop Loss Logic - DISABLED PERMANENTLY
        # Stop loss code removed to prevent any automatic closing on drawdown.
        # DCA will continue to safety orders until TP is hit.
...
def start_strategy():
    try:
        ...
        dca_config = {
            ...
            "stop_loss": 14.0,
            "so_cooldown_sec": 60,
            "trailing_take_profit": False, # DISABLED
...
def create_bot():
    try:
        ...
        dca_config.setdefault('stop_loss_enabled', False) # DISABLED
        dca_config.setdefault('stop_loss', 14.0)
        dca_config.setdefault('so_cooldown_sec', 60)
        dca_config.setdefault('trailing_take_profit', False) # DISABLED
